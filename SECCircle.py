...
lamb = np.empty(2*L+1, dtype = float)
...
dphi_even = lambda i, x: -np.sqrt(2)*i*np.sin(i*x/2)/2
dphi_odd = lambda i, x: np.sqrt(2)*(i+1)*np.cos((i+1)*x/2)/2


# Apply analysis operator T to obtain v_hat_prime
v_hat_prime = np.empty([2*L+1, 2*L+1], dtype = float)
    ...
    if i == 0:
        for j in range(0, 2*L+1):
            if j == 0:
                ...
                v_hat_prime[i,j] = quad(inner_prod, -np.pi, np.pi)[0]
            else:
...
c = np.empty([2*L+1, 2*L+1, 2*L+1], dtype = float)
for i in range(0, 2*L+1):
            if i == 0:
                for j in range(0, 2*L+1):
                    if j == 0:
                        for k in range(0, 2*L+1):
                              if k == 0:
                                    triple_prod = lambda x: 1
                                    c[i,j,k] = quad(triple_prod, -np.pi, np.pi)[0]
                              elif (k % 2) == 0 and k!= 0:
...
g = np.empty([2*L+1, 2*L+1, 2*L+1], dtype = float)
for i in range(0, 2*L+1):
            for j in range(0, 2*L+1):
                        for k in range(0, 2*L+1):
...
G = np.empty([2*L+1, 2*L+1, 2*L+1, 2*L+1], dtype = float)
G = np.einsum('mik,mjl->ijkl', c, g, dtype = float)
G_dual = np.linalg.pinv(G)

# print(np.isnan(G).any())
# print(np.isinf(G).any())
# print(G_dual[1,1,1,1])


# Apply dual Gram operator G^+ to obtain v_hat 
v_hat = np.einsum('ijkl,ij->kl', G_dual, v_hat_prime, dtype = float)
...
w_m = np.empty([2*L+1, 2], dtype = float)
F_1_coeff = np.empty(2*L+1, dtype = float)
F_2_coeff = np.empty(2*L+1, dtype = float)
...
w_phi_theta_x = np.empty([10, 2*L+1], dtype = float)
w_phi_theta_y = np.empty([10, 2*L+1], dtype = float)
...
plt.figure()
ax = plt.gca()
ax.quiver(X, Y, U, V, angles = 'xy', scale_units = 'xy', scale = 200, color = 'red')
ax.set_xlim([-5,5])
ax.set_ylim([-5,5])
